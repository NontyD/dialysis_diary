...
class CommunityTests(TestCase):
    ...
    def test_community_page_post_valid(self):
        ...
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('community'), {'content': 'New test post'})
    ...
    def test_community_page_post_invalid(self):
        ...
        self.assertEqual(Post.objects.count(), 1)  # No new post should be created
        self.assertRedirects(response, reverse('community'))
    ...
    def test_edit_post_post_valid(self):
        ...
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('edit_post', args=[self.post.id]), {'content': 'Updated test post'})
    ...
    def test_edit_post_post_invalid(self):
         ...
         messages_list = list(messages.get_messages(response.wsgi_request))
         self.assertEqual(len(messages_list), 1)
         self.assertEqual(str(messages_list[0]), "Post content cannot be empty.")
    ...
    def test_edit_post_unauthorized(self):
        ...
        messages_list = list(messages.get_messages(response.wsgi_request))
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(str(messages_list[0]), "You can only edit your own posts.")
    ...
    def test_delete_post_unauthorized(self):
        ...
        messages_list = list(messages.get_messages(response.wsgi_request))
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(str(messages_list[0]), "You can only delete your own posts.")
